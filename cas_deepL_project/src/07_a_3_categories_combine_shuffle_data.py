...
def copy_images_labels(training_dir, training_dir_remapped, category, split):

    logging.info(f"started copy_images_labels for {category} {split} \n")
    ...
    labels_src = os.path.join(training_dir, f"{category}_{split}_labels.npy")
    labels_dst = os.path.join(training_dir_remapped, f"{category}_{split}_labels.npy")
    shutil.copy(labels_src, labels_dst)
    logging.info(f"Copied {labels_src} to {labels_dst}")

    logging.info(f"finished copy_images_labels for {category} {split} \n")
...
def combine_shuffle_image_label_data(training_dir_remapped, split):
    ...
    industrial_images = np.load(f"{training_dir_remapped}/Industrial_{split}.npy")
    industrial_labels = np.load(f"{training_dir_remapped}/Industrial_{split}_labels.npy")
    
    # Combine data
    combined_images = np.concatenate((herbveg_images, residential_images, industrial_images), axis=0)
    combined_labels = np.concatenate((herbveg_labels, residential_labels, industrial_labels), axis=0)
    ...
    np.save(f"{training_dir_remapped}/three_category_{split}_images.npy", combined_images)
    np.save(f"{training_dir_remapped}/three_category_{split}_labels.npy", combined_labels)
...
def test_images_and_labels(data_dir, split):
    ...
    try:
        ...
        images = np.load(f"{data_dir}/three_category_{split}_images.npy")
        labels = np.load(f"{data_dir}/three_category_{split}_labels.npy")
...
def main():
    try:
        ...
        log_file = os.path.join(log_dir, f"07_a_3_categories_combine_shuffle_data_{timestamp}.log")
        ...
        logging.info("\n")
        logging.info("called via 07_a_3_categories_combine_shuffle_data.py...\n")
        logging.info(" Script Started ...\n")
        logging.info("This script will combine and shuffle the data for the two chosen categories: Residential, HerbaceousVegetation and Industrial.\n")

        # Check if image size is passed as an argument
        if len(sys.argv) != 2:
            print("Usage: python 07_a_3_categories_combine_shuffle_data.py <image_size>")
            sys.exit(1)
        ...
        for category in categories:
            for split in splits:
                copy_images_labels(training_dir, training_dir_remapped, category, split)
