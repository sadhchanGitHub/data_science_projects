...
def load_category(args):
    ...
    try:
        X_data = np.load(os.path.join(preprocessed_dir, f"{category}_{split}.npy"), mmap_mode='r')
        y_data = np.load(os.path.join(preprocessed_dir, f"{category}_{split}_labels.npy"), mmap_mode='r')
        ...
        return np.empty((0, 513, 513, 3)), np.empty((0,))  # Adjust for your image size
