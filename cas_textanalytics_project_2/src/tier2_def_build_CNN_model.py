...
# Convolutional Neural Network (CNN) model designed for text classification, specifically for binary sentiment analysis 
...
def train_model(model, X_train, y_train, X_val, y_val, batch_size=32, epochs=20):
    ...
    log_filename = f'../logs/tier2_01_execute_CNN_model_training_3_training_log_{timestamp}.log'
    ...
    try:
        ...
        logging.info(f"cnn model for text defined in tier2_def_build_CNN_model Training started at {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
        
        # Define callbacks
        early_stopping = EarlyStopping(monitor='val_loss', patience=5, restore_best_weights=True)
        model_checkpoint = ModelCheckpoint(f'../models/tier2_01_cnn_model_{timestamp}.keras', save_best_only=True, monitor='val_loss')
        ...
        # End logging
        logging.info(f"\n cnn model for text defined in tier2_def_build_CNN_model Training ended at {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
    finally:
...
def visualize_model_training(history, timestamp):
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'../outputs/tier2_01_execute_CNN_model_training_progress_{timestamp}.png')
    logging.info(f"Training progress visualized and saved as '../outputs/tier2_01_execute_CNN_model_training_progress_{timestamp}.png'.")
